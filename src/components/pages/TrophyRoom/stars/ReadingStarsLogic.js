...
function getReadingStars(value) {
  switch (value) {
    case 0:
...
export default function ReadingStarsLogic({ value }) {
  return (
    <div>
      {getReadingStars(value).map(value => (
        //<img src={`/static/media/${value}.e0ba5ce1.png`} width={100} />

        <img src={getReadingStar(value)} width={40} alt="stars_image" />
      ))}
    </div>
  );
}
