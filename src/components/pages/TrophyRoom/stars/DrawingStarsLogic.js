...
function getDrawingStars(value) {
  switch (value) {
    case 0:
...
export default function DrawingStarsLogic({ value }) {
  return (
    <div>
      {getDrawingStars(value).map(value => (
        //<img src={`/static/media/${value}.e0ba5ce1.png`} width={100} />

        <img src={getDrawingStar(value)} width={40} alt="stars_image" />
      ))}
    </div>
  );
}
